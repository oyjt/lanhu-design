...
- 精简 CLI 与兼容脚本：统一版本来源和凭据解析，移除无效参数，将 `doctor` 收敛为登录状态兼容入口，并使用原生请求超时能力。
- 统一 `--timeout` 参数单位为秒，并明确区分 stderr 进度信息与 stdout JSON 结果。
- 优化安装向导认证提示：已有有效凭据直接复用，完成信息明确区分复用凭据、读取浏览器和新登录。
- 优化认证交互：导入和退出在普通终端显示可读结果，打开浏览器前的倒计时改为同一行刷新。
