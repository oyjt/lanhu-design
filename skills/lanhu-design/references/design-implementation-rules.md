...
若目标项目根目录存在 `DESIGN.md`（或项目文档中明确指定承担同一职责的设计说明文件），在解析规格、拆分组件与落盘代码前**必须先读取**，并在全流程遵守其中约定。其约束范围包括但不限于：

- 设计令牌与主题（色板、语义色、CSS 变量命名）
- 排版与刻度（字阶、行高、间距与圆角阶梯）
- 复用组件与变体约定
- 目录结构、文件与类名命名惯例
...
- 图层级的具体数值（尺寸、颜色、定位、字号）仍以 `get_design_specs.mjs` 返回的 HTML+CSS / `design_tokens` 为唯一权威来源。
- `DESIGN.md` 负责在**视觉等价**前提下，将实现映射到系统 token、既有基础组件与共享样式。

**冲突处理：** 若 `DESIGN.md` 中已写明冲突策略，按文档执行；否则以规格数值优先保证当前屏还原度，并在交付时向用户说明取舍。
...
| 中间产物 | 典型路径 | 决策 | 理由 |
|---------|---------|------|------|
| 设计图原图 | `download_design_images.mjs --output <dir>` | **确认后删除** | 仅用于还原期间的视觉核对；代码交付后通常无引用，留在项目里可能被误提交 |
| 规格 HTML + 预览资源 | `get_design_specs.mjs --output <dir>` 的 HTML 和 `assets/slices/` | **确认后删除** | 仅是核对渲染用的预览副本；真正的切图已下载到项目资源目录，HTML 里的相对路径通常不被目标代码引用 |
| 切图元数据 JSON | `get_design_slices.mjs > slices.json` | **确认后删除** | 一次性中间格式，下载完成后通常不再需要 |
| 命名映射 | `--name-map names.json` | **确认后删除** | 同上 |
| 下载的切图文件 | 项目资源目录（`src/assets/` 等） | **保留** | 生成代码直接引用，是最终交付物的一部分 |

执行规则：

1. 先确认生成代码中没有引用候选路径（检查 `src`、`url()` 和 import 路径），并列出准备删除的确切文件或目录。
2. 删除属于高风险操作；必须取得用户明确确认。未确认、用户要求保留或引用关系不确定时，不删除并报告路径。
3. 确认后只删除已列出的中间产物，完成后告知用户删除范围和是否可恢复。
4. 默认把中间产物输出到独立目录（如 `.lanhu/` 或系统临时目录），避免散落项目各处；不要把可能包含用户文件的既有目录作为整体清理目标。
5. 如果项目使用 git，可在开始工作前建议把中间目录加入 `.gitignore`，比事后删除更稳妥。
